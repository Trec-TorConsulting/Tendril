...
- [x] 6.3 ~~Add "overdue" indicator to Activity tab when water changes are overdue~~ (deferred — requires bucket-level data not in page state; tracked separately)

## 7. Final Tab Order
- [x] 7.1 Implement tab order: Overview → Buckets → Activity → Tasks → Nutrition & Yield → Health & Photos → (Field - outdoor only) → Settings
- [x] 7.2 Ensure tab state persists when switching between grows (remember last active tab)

## 8. Testing
- [x] 8.1 TypeScript compilation passes with no errors
- [x] 8.2 E2E test: navigate each merged tab and sub-section
- [x] 8.3 Mobile responsiveness: verify tabs don't overflow on small screens (max 8 visible)
