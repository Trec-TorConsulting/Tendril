...
def _fmt_prev_eval(prev: dict, *, filter_feeding: bool = False) -> str:
    ...
    if prev.get("issues"):
        ...
        issue_strs = [i if isinstance(i, str) else i.get("message", i.get("issue", str(i))) for i in issues]
        if filter_feeding:
            issue_strs = [i for i in issue_strs if "schedule" not in i.lower() and "feeding" not in i.lower()]
        if issue_strs:
            parts.append("  Issues:")
            for issue in issue_strs:
                parts.append(f"    - {issue}")
    if prev.get("actions"):
        actions = prev["actions"]
        # Normalize: actions can be list[str] or list[dict]
        action_strs = [a if isinstance(a, str) else a.get("message", a.get("action", str(a))) for a in actions]
        if filter_feeding:
            action_strs = [a for a in action_strs if "schedule" not in a.lower() and "feeding" not in a.lower()]
        if action_strs:
...
async def build_health_check_prompt(
    ...
    system += (
        ...
        "1. **Overall Health Score** (0-100): This score reflects the PLANT'S ACTUAL HEALTH — not the grower's record-keeping. "
        "Base the score on:\n"
        "   - Sensor readings (pH, EC, water temp, dissolved oxygen) — how close to optimal?\n"
        "   - Environmental conditions (temp, humidity, VPD) — is the environment suitable?\n"
        "   - Visual symptoms (from observations/camera) — does the plant look healthy?\n"
        "   - Nutrient status — is feeding appropriate for the stage?\n"
        "   - Root health — any signs of disease?\n"
        "   Score guidelines: 85-100 = thriving, 70-84 = good with minor issues, 55-69 = needs attention, "
        "40-54 = significant problems, below 40 = critical/dying plant.\n"
        "   IMPORTANT: Missing documentation, incomplete journal entries, overdue logging tasks, and unmeasured data points "
        "are NOT plant health problems. They reduce your CONFIDENCE in the assessment but do NOT lower the score. "
        "A plant can be perfectly healthy even if the grower hasn't logged every detail. "
        "If you lack data to assess something, state your confidence level — don't assume the worst.\n"
        "2. **Issues Found**: List problems in TWO categories:\n"
        "   a) PLANT HEALTH issues — actual problems with the plant or its environment (sensor readings out of range, "
        "visual symptoms, environmental stress, pests, pathogens, nutrient imbalances). These affect the score.\n"
