...
- **Post-deploy smoke test** — New step on the `Deploy` job in `.github/workflows/ci.yml` that hits the production ingress after `kubectl rollout status` returns. Verifies `https://api.tendrilgrow.com/health`, `/health/ready`, `/v1/status`, and `https://tendrilgrow.com/` with six 10-second retries each to absorb pod warmup. Failure marks the deploy job red so on-call gets a clear signal that the pods are "Ready" per kubelet but not actually serving traffic correctly (DB unreachable, env-var typo, ingress misroute, etc.).
- **MQTT ingest idempotency** — New `app/mqtt/dedup.py` module with a `claim_message(tenant_id, device_id, sensor_type, payload_bytes)` check that drops duplicate sensor payloads within a 60s window. Catches QoS-1 redeliveries (broker reconnect, worker crash-and-retry) and firmware re-sends without requiring a firmware change. Backed by Redis `SET NX EX` for atomic check-and-claim across worker replicas, with the same in-memory fallback pattern as `app.automation.suppression`. Wired into `app/mqtt/handlers.py:handle_sensor_message` ahead of all DB work so duplicates also skip alert evaluation. Window controlled by `MQTT_DEDUP_SECONDS` (default 60; set to 0 to disable). Added to mypy strict subset.
- **Migrations audit regression guard** — New `tests/unit/test_migrations_audit.py` scans every Alembic migration under `api/migrations/versions/` for the PR #196 bug class: bulk INSERTs (raw SQL, `sa.table().insert()`, or `op.bulk_insert`) that omit the `id` column when the target table has a UUID primary key without `server_default=gen_random_uuid()`. Fails on any future regression. Audit corpus at adoption: 94 tables, 92 UUID PKs, 32 of those without a server default — these tables MUST have `id` populated explicitly in any migration-time bulk insert.

### Performance
- **`app.automation.engine.evaluate_rules` N+1 eliminated** — Previously issued `1 + R + (R × B)` queries per scheduler tick (one rules fetch, one bucket fetch per rule, one latest-reading fetch per bucket per rule). After PR #194 seeded 47 system-default rules per tenant this grew to ~800 round trips for a single small tenant on every tick. Rewritten to: (1) fetch all enabled rules, (2) fetch latest reading per bucket per distinct scope via a single Postgres `DISTINCT ON (Bucket.id) ORDER BY Bucket.id, recorded_at DESC` query, (3) match in memory. Real-world tenant goes from ~800 queries to **2**. New `tests/unit/test_automation_engine.py` includes a SQLAlchemy `after_cursor_execute`-based regression guard that asserts the query count stays bounded.
- **Integration connector retry/backoff** — New `app/integrations/connectors/retry.py` with `retry_request(send, description, ...)` helper. Wraps a zero-arg `httpx` request factory in exponential backoff with full jitter (default 3 attempts at 1s/2s/4s base, capped at 8s, max retry-after capped at 30s). Retries on `ConnectError`/`ConnectTimeout`/`ReadTimeout`/`WriteTimeout`/`PoolTimeout`/`RemoteProtocolError`, 5xx, and 429 (honouring the `Retry-After` header). Never retries 4xx other than 429 or unrelated exceptions. Pilot wired into all 5 outbound HTTP call sites in `connectors/vivosun.py` (auth, token validation, encrypted POST, scene-map fetch, device discovery). Type-checked under mypy strict. Future PRs migrate the other 6 connectors (ecowitt, home_assistant, opensprinkler, openweather, pulse, tuya).
