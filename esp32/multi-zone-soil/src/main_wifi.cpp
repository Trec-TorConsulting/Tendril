/// @file main_wifi.cpp
/// Tendril Multi-Zone Soil — WiFi + MQTT firmware.
...
void setup() {
    ...
    Serial.println("\n════════════════════════════════════════");
    Serial.printf("  Tendril Multi-Zone Soil  v%s\n", TENDRIL_FW_VERSION);
    ...
    tendril::ZoneReading zones[NUM_ZONES] = {};
    tendril::sensors_read_zones(zones, NUM_ZONES);
    auto ambient = tendril::sensors_read_ambient();

    for (int i = 0; i < NUM_ZONES; i++) {
        Serial.printf("[data] Zone %d: %d%% moisture", zones[i].zone, zones[i].moisture_pct);
        if (zones[i].temp_valid) Serial.printf(", %.1f°F", zones[i].soil_temp_f);
        Serial.println();
    }
    ...
    char payload[512];
    tendril::zones_to_json(zones, NUM_ZONES, payload, sizeof(payload));
